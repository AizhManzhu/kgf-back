...
//        if (is_resource($pubkeyid)) {
            ...
            \Illuminate\Support\Facades\Log::debug(is_resource($result));
            return $result;
//        };
//        return 3;
    }
...
function process_request($order_id, $currency_code, $amount, $config_file, $b64 = true)
{
    ...
    $request = array();
    $request['MERCHANT_CERTIFICATE_ID'] = 'b338e9936774214b';
    $request['MERCHANT_NAME'] = 'CENT.KZ';
    $request['ORDER_ID'] = $order_id;
    $request['CURRENCY'] = $currency_code;
    $request['MERCHANT_ID'] = 92399971;
    $request['AMOUNT'] = $amount;

    $kkb = new KKBSign();
    $kkb->invert();
    if (!$kkb->load_private_key(public_path(env('EPAY_CERT_PATH')) . "/cert.prv", $config['PRIVATE_KEY_PASS'])) {
        if ($kkb->ecode > 0) return $kkb->estatus;
    }
    $result = process_XML(public_path(env('EPAY_CERT_PATH'))."/template.xml", $request);
...
function process_response($response, $config_file)
{
    ...
    if (in_array("DOCUMENT", $result)) {
        ...
        $check = $kkb->check_sign64($data['LETTER'], $data['RAWSIGN'], public_path(env('EPAY_CERT_PATH')) . "/kkbca.pem");
        if ($check == 1)
            $data['CHECKRESULT'] = "[SIGN_GOOD]";
        elseif ($check == 0)
